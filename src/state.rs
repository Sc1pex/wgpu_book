...
impl State {
    ...
    pub fn update(&mut self) {
        ...
        let current_time = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_millis() as u32;
        let angle: cgmath::Vector3<f32> = (1.0, 0.4, 0.2).into();
        self.model_matrix = cgmath::Matrix4::from_axis_angle(
            angle.normalize(),
            cgmath::Deg(current_time as f32 / 10.0),
        );
        self.queue.write_buffer(
            &self.model_buffer,
            0,
            bytemuck::cast_slice(&[<cgmath::Matrix4<f32> as Into<[[f32; 4]; 4]>>::into(
                self.model_matrix,
            )]),
        );
    }
